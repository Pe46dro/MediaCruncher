...
- Persistence write failures cause the file to be requeued with a retry, allowing the evaluation to be redone if needed. The rule set is cached in memory between evaluations to avoid repeated loading.
